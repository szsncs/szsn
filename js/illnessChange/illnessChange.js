...
var illness = {
	...
	initCallAction : function() {//新增初始化
		...
		$service.callAction({
			...
			"action" : "init", //方法名,
			"params" : json, //自定义参数
			"callback" : "initAddcallBack()", //请求回来后执行的ActionID
	...
	infoInitCallAction : function(){//详情初始化接口
		...
		$service.callAction({
			...
			"action" : "infoInit", //方法名,
			"params" : json, //自定义参数
			"callback" : "infoInitcallBack()", //请求回来后执行的ActionID
			"error" : "erresg()"//失败回调的ActionId
		});
	...
	/*createItem : function(billinfo,i){
		if(!billinfo){
			return
		}
		$("#list_div").html("");
		var html=""
		html+='		<li id="illness_list'+i+'">'
			+'		<div class="um-list-item">'
			+'			<div class="um-list-item-inner">'
			+'				<div class="um-list-item-left pl15" style="-webkit-box-align: '
			+'				start;-webkit-align-self: flex-start; width:30%">'
			+'					病变情况：'
			+'				</div>'
			+'				<div class="illnessChange" >'
			+'					<select id="illness_type'+i+'" tag="'+i+'" class="select_type">'
			+'					</select>'
			+'				</div>'
			+'			</div>'
			+'		</div>'
			...
			if(i!="0"){
				var no=--i;
				$(html).insertAfter("#illness_list"+no);
			}
		$("#list_div").html(html);
		illness.append(billinfo,i);
	},*/
